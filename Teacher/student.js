...
function reuslt_by_user(id){
    ...
    results.forEach((result,index) => {
        if(result.studentId==id){
            const exam=exams.find(e=>(e.id!==undefined?e.id:e.examId)===result.examId); // look up by id, not array position
            ...
            text+=`<tr>
                ...
                <td><div class="status ${status=="pass"?"pass":"Failed"}">${status}</div></td>
                <td>${date}</td>
...
function answer_by_exam(id){
    ...
    const exam=exams.find(e=>(e.id!==undefined?e.id:e.examId)===results[id].examId); // look up by id, not array position
    const answers=results[id].answers||results[id].studentAnswers||[]; // students save answers, seed saves studentAnswers
    answers.forEach((Answer,index)=>{
         const q=exam?exam.questions.find(x=>(x.id!==undefined?x.id:x.questionId)===Answer.questionId):null; // match the question by id
         const given=Answer.answer!==undefined?Answer.answer:Answer.userAnswer; // students save answer, seed saves userAnswer
         text+=`<tr>
            <td>${Answer.questionId}</td>
            <td>${q?(q.text!==undefined?q.text:q.questionText):"-"}</td>
